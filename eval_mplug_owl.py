...
sys.path.append(os.path.join(os.path.dirname(__file__), 'mPLUG-DocOwl', 'DocOwl1.5'))
...
def run_model(args, samples, model, call_model_engine_fn=None, tokenizer=None, processor=None,
    ...
    with torch.no_grad():
        for sample in tqdm(samples):
            ...
            response = docowl.inference(img, query)
            sample['image'] = None
...
def conv2sample(args, convs, id, ds, image, _shots=None, load_image=False):
    ret = []
...
def main():
    ...
    tokenizer, model, vis_processors = None, None, None
